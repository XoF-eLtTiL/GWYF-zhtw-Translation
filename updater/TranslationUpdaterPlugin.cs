...
public sealed class TranslationUpdaterPlugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        EnsureFileExists(_readmePath,
            ...
            "3. Translation files sync to BepInEx/Translation." + Environment.NewLine +
            "4. The updater writes progress to status.txt.");
    ...
    private string GetLocalPath(ManifestEntry entry)
    {
        ...
        return Path.Combine(_translationRoot, relativePath.Replace('/', Path.DirectorySeparatorChar));
    }

    private string BuildRemoteFileUrl(ManifestEntry entry)
    {
        string relativePath = NormalizeRelativePath(entry.RelativePath);
        return BuildRawFileUrl(_rawBaseUrl.Value.Trim(), relativePath);
    }
