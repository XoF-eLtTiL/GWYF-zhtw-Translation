...
[BepInPlugin("codex.gwyf.translationupdater", "GWYF Translation Updater", "0.1.2")]
public sealed class TranslationUpdaterPlugin : BaseUnityPlugin
{
    ...
    private ConfigEntry<string> _translationRootPath = null!;
    ...
    private const string ManifestUrl = "https://raw.githubusercontent.com/XoF-eLtTiL/GWYF-zhtw-Translation/refs/heads/main/manifest.txt";
    private const string RawBaseUrl = "https://raw.githubusercontent.com/XoF-eLtTiL/GWYF-zhtw-Translation/refs/heads/main/translations";
    private const long MaxDownloadBytes = 25 * 1024 * 1024;
    private static readonly string[] AllowedPathPrefixes = { "zh-TW/Text/", "zh-TW/Texture/" };
    ...
    private void Awake()
    {
        ...
        _translationRootPath = Config.Bind("Paths", "TranslationRoot", "Translation", "Translation root relative to BepInEx. Use config\\Translation for Thunderstore profiles.");

        _translationRoot = ResolveBepInExRelativePath(_translationRootPath.Value);
        ...
        EnsureFileExists(_readmePath,
            ...
            "1. Remote source is locked to XoF-eLtTiL/GWYF-zhtw-Translation." + Environment.NewLine +
            "2. Put 'update' or 'force' into commands.txt to trigger a sync." + Environment.NewLine +
            "3. Only zh-TW/Text/*.txt and zh-TW/Texture/*.png are downloaded." + Environment.NewLine +
            $"4. Files are written to {_translationRoot}." + Environment.NewLine +
            "5. The updater only writes files; it never executes downloaded content.");

        DeleteAutoGeneratedTranslationFiles();
    ...
    private async Task RunUpdateAsync(bool force, string reason)
    {
        try
        {
            ...
            string manifestText = await HttpClient.GetStringAsync(ManifestUrl);
            ...
            {
                if (!IsSafeManifestEntry(entry, out string safetyReason))
                {
                    Logger.LogWarning($"Skipped unsafe manifest entry '{entry.RelativePath}': {safetyReason}");
                    continue;
                }

                string localPath = GetLocalPath(entry);
                if (!IsPathUnderRoot(localPath, _translationRoot))
                {
                    Logger.LogWarning($"Skipped manifest entry outside translation root: {entry.RelativePath}");
                    continue;
                }
                ...
                string remoteUrl = BuildRemoteFileUrl(entry);
                byte[] content = await HttpClient.GetByteArrayAsync(remoteUrl);
                if (content.LongLength != entry.Size)
                {
                    WriteStatus($"Size mismatch for {entry.RelativePath}. Skipped.");
                    continue;
                }
            ...
            if (downloaded > 0)
            {
                TouchTranslationFiles();
                TryReloadXUnityTranslations();
            }
    ...
    private string GetLocalPath(ManifestEntry entry)
    {
        string relativePath = NormalizeRelativePath(entry.RelativePath);
        return Path.GetFullPath(Path.Combine(_translationRoot, relativePath.Replace('/', Path.DirectorySeparatorChar)));
    }
    ...
    private string BuildRemoteFileUrl(ManifestEntry entry)
    {
        string relativePath = NormalizeRelativePath(entry.RelativePath);
        return BuildRawFileUrl(RawBaseUrl, relativePath);
    }
    ...
    private void DeleteFilesMissingFromManifest(List<ManifestEntry> remoteEntries)
    {
        ...
        if (Directory.Exists(_translationRoot))
        {
            foreach (string file in Directory.GetFiles(_translationRoot, "*", SearchOption.AllDirectories))
            {
                ...
                if (IsSafeRelativeDownloadPath(relative) && !allowed.Contains(relative))
                {
                    File.Delete(file);
    ...
    private void DeleteAutoGeneratedTranslationFiles()
    {
        if (!Directory.Exists(_translationRoot))
        {
            return;
        ...
        foreach (string file in Directory.GetFiles(_translationRoot, "_AutoGeneratedTranslations.txt", SearchOption.AllDirectories))
        {
            File.Delete(file);
            Logger.LogInfo($"Deleted auto-generated translation file: {NormalizeRelativePath(GetRelativePathCompat(_translationRoot, file))}");
        }
    }

    private void TouchTranslationFiles()
    {
        if (!Directory.Exists(_translationRoot))
        {
            return;
        ...
        {
            string relative = NormalizeRelativePath(GetRelativePathCompat(_translationRoot, file));
            if (!IsSafeRelativeDownloadPath(relative))
            {
                continue;
            }
            ...
            {
                Logger.LogWarning($"Could not touch translation file for reload: {relative}. {ex.Message}");
            }
        }
    }

    private void TryReloadXUnityTranslations()
    {
        try
        {
            Type? initializerType = Type.GetType("XUnity.AutoTranslator.Plugin.Core.SettingsTranslationsInitializer, XUnity.AutoTranslator.Plugin.Core");
            MethodInfo? loadTranslations = initializerType?.GetMethod("LoadTranslations", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
            if (loadTranslations == null)
            {
                Logger.LogInfo("XUnity translation reload hook was not found; updated translations may apply after text refresh or game restart.");
                return;
            }

            loadTranslations.Invoke(null, null);
            Logger.LogInfo("Requested XUnity translation reload after update.");
        }
        catch (Exception ex)
        {
            Logger.LogWarning($"Could not request XUnity translation reload; updated translations may apply after text refresh or game restart. {ex.Message}");
        }
    }
    ...
    private static bool IsSafeManifestEntry(ManifestEntry entry, out string reason)
    {
        string relativePath = NormalizeRelativePath(entry.RelativePath);
        if (!IsSafeRelativeDownloadPath(relativePath))
        {
            reason = "path is outside the locked translation file allowlist";
            return false;
        }

        if (!IsSha256Hex(entry.Sha256))
        {
            reason = "sha256 is invalid";
            return false;
        }

        if (entry.Size < 0 || entry.Size > MaxDownloadBytes)
        {
            reason = "size is invalid";
            return false;
        }
    ...
    private static bool IsSafeRelativeDownloadPath(string relativePath)
    {
        if (string.IsNullOrWhiteSpace(relativePath) ||
            relativePath.StartsWith("/", StringComparison.Ordinal) ||
            relativePath.StartsWith("\\", StringComparison.Ordinal) ||
            relativePath.IndexOf(':') >= 0 ||
            relativePath.Split('/').Any(part => part == "." || part == ".." || string.IsNullOrWhiteSpace(part)))
        {
            return false;
        }

        if (!AllowedPathPrefixes.Any(prefix => relativePath.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)))
        {
            return false;
        }

        return AllowedExtensions.Contains(Path.GetExtension(relativePath));
    }

    private static bool IsPathUnderRoot(string candidatePath, string rootPath)
    {
        string fullCandidate = Path.GetFullPath(candidatePath);
        string fullRoot = AppendDirectorySeparator(Path.GetFullPath(rootPath));
        return fullCandidate.StartsWith(fullRoot, StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsSha256Hex(string value)
    {
        return value.Length == 64 && value.All(ch =>
            (ch >= '0' && ch <= '9') ||
            (ch >= 'a' && ch <= 'f') ||
            (ch >= 'A' && ch <= 'F'));
    }
